...
If the above does not complete successfully, make sure the ruby version at the top of `Gemfile` matches your local ruby version (e.g. if you have ruby 3.1.2, your `Gemfile` should have `ruby "3.1.2` at the top.  Then, run `bundle`.
...
Generate a test log file using `rspec -fd -o Lastname_FirstName_HW1.txt spec/`. That puts all the logs into one file.
...
* Try conforming to a Ruby style guide: `gem install rubocop`, then run `rubocop [list of filenames to check]`.
